...
class KDTree
{
  ...
  private:
    ...
    /**
     * Recursive helper function for findNearestNeighbor that
     * takes a point in the kd tree as a parameter
     */
    Point<Dim> _findNearestNeighbor(const Point<Dim>& query, const KDTreeNode* subrootl, int splitDim) const;
	 ...
	 */
	KDTreeNode * tree_creation(vector<Point<Dim>>& newPoints, int left, int right, int dimension);
