                            ...
                            queue<TreeNode*>& mergeQueue)
{
     ...
     */
     ...
     if (result == nullptr)
     {
         std::cout << "*************************removeSmallest returned NULL!!!******************";
     }
     return result;
}
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     // break statement when one queue is empty while the other has one element
     while(true)
     {
       TreeNode* merge1 = removeSmallest(singleQueue, mergeQueue);
       TreeNode* merge2 = removeSmallest(singleQueue, mergeQueue);

       TreeNode* new_merge = new TreeNode(Frequency((merge1 -> freq).getFrequency() + (merge2 -> freq).getFrequency()));
       new_merge -> left = merge1;
       new_merge -> right = merge2;
       mergeQueue.push(new_merge);

       if(singleQueue.empty() && !mergeQueue.empty() &&  mergeQueue.front() == mergeQueue.back())
       {
       ...
       if(mergeQueue.empty() && !singleQueue.empty() &&singleQueue.front() != NULL &&  singleQueue.front() == singleQueue.back())
       {
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.hasBits())
    {
        ...
        {
            result = new TreeNode(Frequency(bfile.getNextByte(), 0));
            result -> left = nullptr;
...
{
    // i added this because the given code is broken?????
