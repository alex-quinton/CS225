...
heap<T, Compare>::heap(const std::vector<T>& elems) 
{
    ...
    _elems = elems;
	_elems.insert(_elems.begin(), T()); // Placeholder for index 0 

	int i = 1;
	while(hasAChild(i+1)) i++;
    while(i > 0)
    {
        heapifyDown(i);
		i--;
...
void heap<T, Compare>::updateElem(const size_t & idx, const T& elem)
{
    ...
    if (higherPriority(_elems[index], _elems[parent(index)]))
    {
        heapifyUp(index);
    } 
    
    if (hasAChild(index) && 
            higherPriority(_elems[maxPriorityChild(index)], _elems[index]))
    {
        heapifyDown(index);
    }
